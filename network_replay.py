...
class cnn_lstm(nn.Module):
  def __init__(self):
    ...
    self.pretrain =nn.Sequential(*list(model.children()))[0:7] ### Resnet #####
    self.prelstm=nn.Sequential(nn.AdaptiveAvgPool2d((1,1)) ,Flatten(),)
    #self.lstm=nn.LSTM(1280,512,3,batch_first=True)
    self.lstm=nn.LSTM(1024,512,3,batch_first=True)#### Resnet ##### 
  ...
  def forward(self,input,frames_per_video):
    ...
    features=self.pretrain(torch.cat(stacked_input))
